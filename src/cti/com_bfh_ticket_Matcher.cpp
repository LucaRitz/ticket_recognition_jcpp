...
const Text* textFactory(JNIEnv* env, jobject& javaText) {
    ...
    auto keyAsJavaString = (jstring) env->CallObjectMethod(javaTextClass, nameMethod);
    ...
    jobject javaBoundingBox = env->CallObjectMethod(javaTextClass, boundingBoxMethod);
...
const Ticket* ticketFactory(JNIEnv* env, jobject& javaTicket) {
    ...
    vector<const Text*> texts = convert(env, javaTextList, textFactory);

    Ticket* ticket = new Ticket(cppName, *ticketImage, texts);
    jmethodID setPointerMethod = env->GetMethodID(javaTicketClass, "setPointer", "(J)");
    env->CallObjectMethod(javaTicket, setPointerMethod, &ticket);
